...
<html>
    ...
    <div class="topnav">
        ...
        <a href="{{ url('/main/messages')}}">Messages</a>
        <a href="{{ url('/main/printinghouse') }}">Printing Houses</a>
        <a href="{{ url('/main/publisher') }}">Publishers</a>
        <a href="{{ url('/main/stores') }}">Stores</a>
        <div class="topnav-right">
            <a class="active" href="{{ url('/main/profile') }}"><i class="fa fa-fw fa-user" style = "color:white; margin : 3px;" ></i>{{Auth::user()->name}}</a>
    ...
    <table class="table table-bordered table-striped">
        ...
        <table class="table table-bordered table-striped">
            ...
            <tr>
                ...
                <td ><a href="{{action('MainController@deleteBookFromProfile',['ISBN' =>$book -> ISBN])}}"><span class="glyphicon glyphicon-trash"></span></a></td>
            </tr>
            @php ($i= $i + 1)
            @endforeach
            </tbody>
        ...
        <table class="table table-bordered table-striped">
            ...
            <tr>
                ...
                <td>{{ $friend->name }}</td>
                <td>{{ $friend->email }}</td>
                <td ><a href="{{action('MainController@deleteFriend',['id' =>$friend -> friendid, 'friendid'=>$friend -> id])}}"><span class="glyphicon glyphicon-trash"></span></a></td>
            </tr>
            @php ($i = $i + 1)
            @endforeach
            </tbody>
